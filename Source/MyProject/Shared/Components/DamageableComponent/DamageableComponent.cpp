    ...
    const FHitResult& Hit)
{
    ...
    if (ACharacter* Character = Cast<ACharacter>(GetOwner()))
    {
        if (UCharacterMovementComponent* CMC = Character->GetCharacterMovement())
        {
            ...
            // Ignorujemy jedynie płaską podłogę pod stopami (Z > 0.7f), ponieważ lądowanie na stopach
            // jest autorytatywnie obsługiwane przez LandedDelegate (unikamy podwójnego naliczania obrażeń).
